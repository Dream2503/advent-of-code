...
std::vector<Group> parse_input(const char* input) {
...
Army simulate(std::vector<Group>& groups) {
    while (std::ranges::count(groups, Army::IMMUNE_SYSTEM, &Group::army) && std::ranges::count(groups, Army::INFECTION, &Group::army)) {
        ...
        for (Group& attacker : groups) {
            if (attacker.target != -1 && attacker.units > 0) {
                ...
                if (defender->units > 0) {
                    const int killed = compute_effective_damage(attacker, *defender) / defender->hit_points;
...
A boost is an integer increase in immune system units' attack damage. For example, if you were to boost the above example's immune system's units by
...
You don't even know how you could boost the reindeer's immune system or what effect it might have, so you need to be cautious and find the smallest
...
int part2() {
    ...
    while (simulate(temp) != Army::IMMUNE_SYSTEM) {
        temp.clear();
        ...
        for (const Group& group : groups) {
            ...
            if (group.army == Army::IMMUNE_SYSTEM) {
                temp.back().damage += buff;
            }
        }
        buff++;
    }
    return std::transform_reduce(temp.begin(), temp.end(), 0, std::plus(), [](const Group& group) -> int { return group.units; });
}

...
Furthermore, you reason, a new lanternfish would surely need slightly longer before it's capable of producing more lanternfish: two more days for its
first cycle.
...
This list means that the first fish has an internal timer of 3, the second fish has an internal timer of 4, and so on until the fifth fish, which has
an internal timer of 2. Simulating these fish over several days would proceed as follows:
...
uint64_t part1(const int iteration = 80) {
    ...
    for (int i = 0; i < iteration; i++) {
        const uint64_t spawn = fishes[0];
        std::ranges::rotate(fishes, fishes.begin() + 1);
        fishes[6] += fishes[8] = spawn;
    }
    return std::ranges::fold_left(fishes, 0, std::plus());
}

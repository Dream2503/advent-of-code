...
uint64_t part1(const char* input, const int iteration) {
    ...
    const int size = polymer.size();

    for (int i = 0; i < size - 1; i++) {
        pair_count[polymer.substr(i, 2)]++;
    }
    ...
    for (int i = 0; i < iteration; i++) {
        ...
        for (const auto& [pair, count] : pair_count) {
            const auto itr = rules.find(pair);

            if (itr != rules.end()) {
                temp[std::string{pair.front(), itr->second}] += count;
                temp[std::string{itr->second, pair.back()}] += count;
            } else {
                temp[pair] += count;
            }
        }
        pair_count = std::move(temp);
    }
    std::array<uint64_t, 26> res{};

    for (const auto& [pair, count] : pair_count) {
        res[pair.front() - 'A'] += count;
    }
    res[polymer.back() - 'A']++;

    const auto [min, max] = std::ranges::minmax(res | std::views::filter([](const uint64_t value) -> bool { return value != 0; }));
    return max - min;
}
...
uint64_t part2(const char* input) { return part1(input, 40); }

int main() {
    std::println("Part 1:");
    Executor::test(part1, R"(NNCB
...
CN -> C)",
                   10, 1588);
    Executor::run(part1, input14, 10);

    std::println("Part 2:");
    Executor::test(part2, R"(NNCB

...
that only leaves the interior nine trees to consider:
    ...
    - The center 3 is not visible from any direction; for it to be visible, there would need to be only trees of at most height 2 between it and an
      edge.
...
int part1(const char* input) {
    ...
    std::vector seen(row, std::vector<uint8_t>(col, false));
    ...
    return std::ranges::fold_left(seen | std::views::join, 0, std::plus());
}
    ...
    for (int i = 1; i < row - 1; i++) {
        for (int j = 1; j < col - 1; j++) {
            ...
            for (const Vec2<int>& delta : directions_basic) {
                int temp = 0;
                Vec2 pos = Vec2(i, j) + delta;

                while (pos.x >= 0 && pos.x < row && pos.y >= 0 && pos.y < col) {
                    temp++;

                    if (grid[pos.x][pos.y] >= grid[i][j]) {
                        break;
                    }
                    pos += delta;
                }

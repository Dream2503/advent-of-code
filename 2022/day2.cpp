...
single round is the score for the shape you selected (1 for Rock, 2 for Paper, and 3 for Scissors) plus the score for the outcome of the round (0 if
you lost, 3 if the round was a draw, and 6 if you won).

Since you can't be sure if the Elf is trying to help you or trick you, you should calculate the score you would get if you were to follow the strategy
guide.
...
int part1(const char* input, const bool part2) {
    ...
    while (std::getline(file, line)) {
        char elf, me;
        std::sscanf(line.c_str(), "%c %c", &elf, &me);
        ...
        if (part2) {
            ...
            if (me - elf == 24 || me - elf == 21 || me - elf == 19) {
                res += me - 'X' + 1;
            } else if (me - elf == 23) {
                res += 3 + me - 'X' + 1;
            } else {

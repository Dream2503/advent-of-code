...
two compartments, so the first half of the characters represent items in the first compartment, while the second half of the characters represent
...
int part1(const char* input) {
    ...
    while (std::getline(file, line)) {
        ...
        auto first = line | std::views::take(size / 2);
        auto second = line | std::views::drop(size / 2);
        std::ranges::sort(first);
        std::ranges::sort(second);
        std::ranges::set_intersection(std::ranges::subrange(first.begin(), std::ranges::unique(first).begin()),
                                      std::ranges::subrange(second.begin(), std::ranges::unique(second).begin()), &ch);
        res += std::islower(ch) ? ch - 'a' + 1 : ch - 'A' + 27;
    }
    return res;
}
...
three Elves, the badge is the only item type carried by all three Elves. That is, if a group's badge is item type B, then all three Elves will have
item type B somewhere in their rucksack, and at most two of the Elves will be carrying any other item type.
...
In the first group, the only item type that appears in all three rucksacks is lowercase r; this must be their badges. In the second group, their badge
item type must be Z.
...
int part2(const char* input) {
    ...
    while (std::getline(file, line1)) {
        ...
        std::ranges::sort(line3);
        std::ranges::set_intersection(line1, line2, std::back_inserter(temp));
        std::ranges::set_intersection(temp, std::ranges::subrange(line3.begin(), std::ranges::unique(line3).begin()), &ch);
        res += std::islower(ch) ? ch - 'a' + 1 : ch - 'A' + 27;
    }
    return res;
}
...
int main() {
    std::println("Part 1:");
    Executor::test(part1, R"(vJrwpWtwJgWrhcsFMMfFFhFp
jqHRNqRjqzjGDLGLrsFMfFZSrLrFZsSL
PmmdzqPrVvPwwTWBwg
wMqvLMZHhHMvwLHjbvcjnnSBnvTQFn
ttgJtRGJQctTZtZT
CrZsJsPPZsGzwwsLwLmpwMDw)",
    ...
    std::println("Part 2:");
    Executor::test(part2, R"(vJrwpWtwJgWrhcsFMMfFFhFp
jqHRNqRjqzjGDLGLrsFMfFZSrLrFZsSL
PmmdzqPrVvPwwTWBwg
wMqvLMZHhHMvwLHjbvcjnnSBnvTQFn
ttgJtRGJQctTZtZT

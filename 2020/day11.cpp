...
Now, you just need to model the people who will be arriving shortly. Fortunately, people are entirely predictable and always follow a simple set of
rules. All decisions are based on the number of occupied seats adjacent to a given seat (one of the eight positions immediately up, down, left, right,
or diagonal from the seat). The following rules are applied to every seat simultaneously:
...
int part1(const bool part2 = false) {
    ...
    for (const char ch : input11) {
        if (ch == '\n') {
            ...
            prev.back().push_back(static_cast<Seat>(ch));
        }
    }
    const int size1 = prev.size(), size2 = prev.front().size();
            ...
            for (int j = 0; j < size2; j++) {
                ...
                for (const Vec2<int>& delta : directions_complete) {
                    ...
                    while (true) {
                        ...
                        if (next.x < 0 || next.x >= size1 || next.y < 0 || next.y >= size2) {
                            break;
                        }
                        ...
                        if (current[next.x][next.y] == Seat::OCCUPIED) {
                            occupied++;
                        }
                        break;
                    }
    ...
    return std::ranges::count(prev | std::views::join, Seat::OCCUPIED);
}
...
Also, people seem to be more tolerant than you expected: it now takes five or more visible occupied seats for an occupied seat to become empty (rather
than four or more from the previous rules). The other rules still apply: empty seats that see no occupied seats become occupied, seats matching no

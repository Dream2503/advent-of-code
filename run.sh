    ...
    for file in "$year"/day{1..25}.cpp; do
        if [[ -f "$file" ]]; then
            ...
            if $optimization; then
                g++ -std=c++23 -include utils.hpp -Wall -Wextra -Wpedantic -Wfloat-equal -Wcast-align -Wold-style-cast -Woverloaded-virtual -Wnon-virtual-dtor -Wduplicated-cond -Wduplicated-branches -Wlogical-op -Wuseless-cast -Wformat=2 -Wsuggest-override -Wdouble-promotion -Wswitch-enum -Wformat-security -Wformat-overflow -Winvalid-offsetof -Wno-sign-compare -Wno-stringop-overflow -Ofast -march=native -flto=auto -funroll-loops -fomit-frame-pointer -ffast-math -fno-trapping-math -finline-functions -fmerge-all-constants -fstrict-aliasing -falign-functions=32 -falign-loops=32 -falign-jumps=32 -DNDEBUG "$file" -lcrypto -o "$exe"
            else

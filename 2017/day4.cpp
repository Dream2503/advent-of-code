...
A new system policy has been put in place that requires all accounts to use a passphrase instead of simply a password. A passphrase consists of a
...
int part1(const bool anagram = false) {
    ...
    while (std::getline(file, line)) {
        ...
        while (!ss.eof()) {
            ...
            if (std::ranges::find(seen, word) != seen.end()) {
                flag = false;
                break;
            }
            seen.push_back(word);
